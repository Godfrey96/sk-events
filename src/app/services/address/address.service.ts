...
export class AddressService {
  ...
  constructor(
    ...
    // const user = this.authService.currentUser$.subscribe(x => this.user = x);
  }
  ...
  // get my address
  // getMyAddress(userId: string): Observable<Address> {
  //   return this.http.get<Address>(`${this.apiURLAddress}/get/my-address/${userId}`);
  // }

  async getAddressRef(query?) {
    // if (!this.uid) this.uid = await this.getUid();
    return await this.apiService.collection('address').doc(this.uid).collection('all', query);
  }
  ...
  async addAddress(param) {
    try {
      const currentAddress = this._addresses.value;
      const data = new Address(
        this.uid ? this.uid : await this.getUid(),
        this.uid ? this.uid : await '',
        param.title,
        param.address,
        param.landmark,
        param.house,
